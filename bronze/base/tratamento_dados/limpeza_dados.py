...
def validar_dataframe(dataframe: pd.DataFrame) -> None:
    """
    Valida se o DataFrame possui linhas e colunas para limpeza.
    """

    if dataframe.empty:
        raise ValueError("DataFrame vazio. A limpeza não pode ser executada.")

    if dataframe.shape[1] == 0:
        raise ValueError("DataFrame sem colunas. A limpeza não pode ser executada.")
...
def converter_textos_nulos_para_na(dataframe: pd.DataFrame) -> pd.DataFrame:
    ...
    print(f"Total de valores textuais nulos convertidos para pd.NA: {total_convertidos}")
...
def tratar_nulos_colunas_criticas(dataframe: pd.DataFrame) -> pd.DataFrame:
    ...
    for coluna, marcador in MARCADORES_NULOS_CRITICOS_BRONZE.items():
        ...
        print(
            f"Coluna '{coluna}': "
            f"{qtd_nulos} valores nulos preenchidos com '{marcador}'."
        )
