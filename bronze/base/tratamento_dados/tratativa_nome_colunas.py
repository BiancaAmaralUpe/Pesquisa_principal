...
def validar_colunas_duplicadas(dataframe: pd.DataFrame) -> None:
    """
    Valida se a padronização gerou nomes de colunas duplicados.
    """
    ...
    if colunas_duplicadas:
        raise ValueError(
            "A padronização gerou colunas duplicadas. "
            f"Colunas duplicadas: {colunas_duplicadas}"
        )